...
def start_replication(log_end_offsets, peer_broker_view):
    stop = threading.Event()
    def _loop():
        bootstrap = True if peer_broker_view["broker_id"] else False
        _cycle = 0
        while not stop.is_set():
            ...
            if not peer_broker_view["broker_id"]: continue
            print(f"{Color.BLUE}[⟳ REPLICATION | {'BOOTSTRAP' if bootstrap else 'STEADY'}] {BROKER_NAME}{Color.WHITE}")
            payload = {"bootstrap": bootstrap, "replica_id": BROKER_NAME, "follower_offsets": {f"{topic}:{pid}": leo for (topic, pid), leo in log_end_offsets.items()}}
            try:
                response = requests.post(f"http://{peer_broker_view['address']}/replica_fetch", json=payload, timeout=5)
