...
from kafkaesque.zookeeper.znodes import ELECTION_ZNODE, CONTROLLER_ZNODE
from kafkaesque.structs import Color
from kafkaesque.zookeeper.partition_assignment_publisher import PartitionAssignmentPublisher
...
class ControllerElection:
    ...
    def controller_task(self):
        ...
        controller_payload = {"broker_id": self.broker_id, "address": self.address}
        self.zk.create(CONTROLLER_ZNODE, json.dumps(controller_payload).encode("utf-8"), ephemeral=True, makepath=True)

        partition_assignment_publisher = PartitionAssignmentPublisher(self.zk)
        partition_assignment_publisher.start()
