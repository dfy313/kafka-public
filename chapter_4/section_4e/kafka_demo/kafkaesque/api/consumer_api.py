...
class KafkaesqueConsumer:
    ...
    def poll(self, timeout_ms=1000, max_records=500):
        ...
        for tp in self._assigned:
            ...
            fetch_records_response, self.base_url = http_with_retry("GET", self._session, self.base_url, f"/fetch/{topic}/{partition}?offset={start}", self.bootstrap_servers, self.max_retries)
    ...
    def commit(self, offsets: dict[TopicPartition, OffsetAndMetadata]):
        ...
        for tp, metadata in items:
            ...
            _response, self.base_url = http_with_retry("POST", self._session, self.base_url, f"/groups/{self.group_id}/offsets/{tp.topic}/{tp.partition}", self.bootstrap_servers, self.max_retries, payload)
    ...
    def _join_if_needed(self):
        ...
        response, self.base_url = http_with_retry("POST", self._session, self.base_url, f"/groups/{self.group_id}/join", self.bootstrap_servers, self.max_retries, payload)
        ...
        for topic, partition_assignments in sorted(full_assignment.items()):
            for pid, owner in partition_assignments.items():
                ...
                get_offset_response, self.base_url = http_with_retry("GET", self._session, self.base_url, f"/groups/{self.group_id}/offsets/{topic}/{pid}", self.bootstrap_servers, self.max_retries)
    ...
    def _heartbeat_loop(self):
        while not self._closed:
            ...
            response, self.base_url = http_with_retry("POST", self._session, self.base_url, f"/groups/{self.group_id}/heartbeat", self.bootstrap_servers, self.max_retries, payload)
