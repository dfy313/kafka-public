...
From the project root (`~/Desktop/kafka_demo`):
...
_Paste in the provided `notification_service` starter code._
...
### 5. Verify Consumer Group Registered Successfully

After launching the e-commerce app, verify that `payment_service` and `notification_service` consumer groups successfully registered with the Kafka broker:
...
Refer back to **[Step 7](#7-verify-order-in-the-database)** for the exact command to display all records in the `Orders` table.
...
Refer back to **[Step 9](#9-inspect-the-internal-__consumer_offsets-topic)** for the exact command to inspect the internal `__consumer_offsets` topic.

### 14. Cleanup: Reset for Future Tests
